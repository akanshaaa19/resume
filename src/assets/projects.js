const projects = [
  {
    index: 1,
    src: "https://i.ibb.co/RzMwcf1/resume.png",
    github: "https://github.com/akanshaaa19/resume-generator",
    active: "https://akanshaaa19.github.io/resume-generator",
    title: "Resume Generator",
    description:
      "This takes up your details and generates a resume, which you can download in PDF format and share.",
    stack: "Redux Tailwind",
  },
  {
    index: 2,
    src: "https://i.ibb.co/BrcKHWh/tasks.png",
    github: "https://github.com/akanshaaa19/tasks",
    active: "https://akanshaaa19.github.io/tasks",
    title: "Tasks App",
    description: 'You can create tasks, check them off, and then delete them in this app. You can also create different lists.',
    stack: "ReactJS ContextAPI",
  },
  // {
  //   index: 3,
  //   src: "https://i.ibb.co/7j5RqXT/portfolio.png",
  //   github: "https://github.com/akanshaaa19/portfolio",
  //   active: "https://akanshaaa19.github.io/portfolio",
  //   title: "Portfolio",
  //   description:"A portfolio application made with ReactJS and Bootstrap that displays all of my skills and the projects I've worked on",
  //   stack: "ReactJS HTML Bootstrap",
  // },
  {
    index: 4,
    src: "https://i.ibb.co/0XJMLsj/weather.png",
    github: "https://github.com/akanshaaa19/weather-app",
    active: "https://akanshaaa19.github.io/weather-app",
    title: "Weather App",
    description:
      "This takes a city name as an input, and returns the current temperature, wind, pressure, and humidity etc.",
    stack: "ReactJS Bootstrap",
  },
];

export default projects;
